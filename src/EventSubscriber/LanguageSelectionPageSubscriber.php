...
class LanguageSelectionPageSubscriber implements EventSubscriberInterface {
  ...
  /**
   * Finds out if a language is available.
   *
   * Loops through all available language negotiation methods with higher
   * priority than the Language Selection Page method itself.
   *
   * @see \Drupal\language_selection_page\LanguageSelectionPage::redirectToLanguageSelectionPage()
   *
   * @return array|bool
  ...
  public function redirectToLanguageSelectionPage(FilterResponseEvent $event) {
    ...
    // Run condition plugins to prevent a cookie from being set.
    ...
    // Assure there is no other language negotiation method that returns
    // a language and has priority over Language Selection page.
    if (!$this->getLanguage()) {
      $request = $this->event->getRequest();

      // Our query parameter is called destination to maintain backward
      // compatibility with the URLs generated by the Drupal 7 module. It is
      // coincidence that this is the same query parameter as the one used by
      // the drupal_get_destination() function from Drupal 7 core.
  ...
  public static function getSubscribedEvents() {
    ...
    // @todo explain why it's -50? just a random number?
